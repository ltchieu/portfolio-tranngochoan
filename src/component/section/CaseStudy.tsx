...
export default function CaseStudy() {
    ...
    // Reset trạng thái play/pause khi chuyển đổi sang Tab case study khác
    useEffect(() => {
        setIsPlaying(true);
        setIsMuted(true);
    }, [activeIdx]);

    // Gửi lệnh điều khiển Play/Pause tới iframe YouTube
    const togglePlay = () => {
    ...
    // Gửi lệnh điều khiển Âm thanh tới iframe YouTube
    const toggleMute = () => {
        if (iframeRef.current?.contentWindow) {
            ...
            <div className="max-w-[1160px] mx-auto">
                ...
                <Reveal delay={120}>
                    <div className="grid grid-cols-1 lg:grid-cols-2 border border-border rounded-[24px] overflow-hidden bg-[#111] shadow-[0_20px_50px_rgba(0,0,0,0.5)]">
                        ...
                        {/* Cột trái: YouTube Iframe & Overlay */}
                        <div className="relative min-h-[300px] lg:min-h-[500px] flex items-center justify-center overflow-hidden border-b lg:border-b-0 lg:border-r border-border bg-[#050505] group">

                            <iframe
                                ref={iframeRef}
                                key={activeCase.videoUrl}
                                className="absolute inset-0 w-full h-full object-cover scale-[1.35] opacity-80 pointer-events-none"
                                src={`https://www.youtube.com/embed/${getYouTubeId(activeCase.videoUrl)}?enablejsapi=1&autoplay=1&mute=1&loop=1&playlist=${getYouTubeId(activeCase.videoUrl)}&controls=0&showinfo=0&rel=0&modestbranding=1&iv_load_policy=3&playsinline=1&vq=hd1080`}
                            ...
                            <div className="absolute inset-0 bg-linear-to-b from-black/60 via-transparent to-black/80 pointer-events-none z-10"></div>

                            {/* Badge Trạng Thái */}
                            <div
                                className="absolute top-5 left-5 inline-flex items-center gap-2 rounded-full px-4 py-2 backdrop-blur-md z-20"
                            ...
                            {/* Nút Điều Khiển Floating (Chỉ giữ phần này) */}
                        ...
                        <div className="p-8 lg:p-12 flex flex-col justify-center">
                            ...
                            <div className="flex flex-wrap items-center gap-3 mt-10">
                                ...
                                <a
                                    href="https://drive.google.com/drive/folders/1kRNsRZ8QEPLElz9kaOMsuWRI0z8ANPZM?usp=drive_link"
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    className="flex items-center gap-2 bg-green text-[#0D0D0D] px-6 py-2.5 rounded-full text-[13px] font-bold transition-all hover:opacity-90 hover:scale-[1.02] shadow-[0_4px_15px_rgba(205,255,90,0.25)] sm:ml-auto w-full sm:w-auto justify-center"
                                >
                                    Kho Tài Nguyên Dự Án (Drive) <FaArrowRight className="rotate-[-45deg]" />
                                </a>
                            </div>
